...
Take $\sigma,\tau\in Hom(\Gamma, P)_{\rho_0}/V$ such that $\sigma = v\cdot\tau$ for some fixed $v\in V$. Then for all $\gamma\in \Gamma$
\begin{eqnarray*}
...
\end{eqnarray*}
Hence by (\ref{equivalentOneCocycles}) $\sigma$ and $\tau$ belong to the same 1-cohomology class. This proves $\overline{h}$ is well-defined. The argument can be run in reverse and since $h$ is bijective, $\overline{h}$ is also. This satisfies our assertions.

Take an element $g\in G$. We can conjugate $\sigma\in Hom(\Gamma, P)_{\rho_0}$ by $g$ to get an element $g\cdot\sigma\in Hom(\Gamma, gPg^{-1})_{g\cdot\rho_0}$. Then $h(g\cdot\sigma) = \alpha_{g\cdot\sigma}$ is an element of $Z^1(\Gamma, g\cdot\rho_0, gVg^{-1})$.

If $g\in P$ then $gPg^{-1} = P$ so conjugating gives rise to the map $Hom(\Gamma, P)_{\rho_0}\rightarrow Hom(\Gamma, P)_{g\cdot\rho_0}$ and $Z^1(\Gamma, g\cdot\rho_0, gVg^{-1})\rightarrow Z^1(\Gamma, g\cdot\rho_0, gVg^{-1})$ under $h$.

Furthermore, if $g\in Z(L)$ then $g\cdot\rho_0 = \rho_0$ and $gVg^{-1} = V$. Then indeed $Z(L)$ acts on $Hom(\Gamma, P)_{\rho_0}$ and also on $Z^1(\Gamma, \rho_0, V)$ via $h$, and the following diagram is commutative:
\begin{displaymath}
	\xymatrix{
