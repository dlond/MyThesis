...
We say $\sigma_1, \sigma_2$ in $Z^1(H, V)$ are equivalent if there exists a $v$ in $V$ such that
\begin{eqnarray}\label{ch4::equivalentOneCocycles}
	\sigma_1(h) = v * \sigma_2(h) * h\cdot v^{-1},
\end{eqnarray}
for all $h$ in $H$. We call the set of equivalence classes of $Z^1(H, V)$ under (\ref{ch4::equivalentOneCocycles}) the \emph{1-cohomology}, denoted $H^1(H, V)$.
...
\end{displaymath}
defined in the usual way under the canonical projection $SL_2 \rightarrow SL_2/B$. This map is well defined and is a morphism [ref Borel]. Now since $SL_2/B$ is an irreducible projective variety [ref Humphreys], $\tilde{\sigma}$ must be constant [ref Borel]. Hence, as $\sigma$ takes the value 1 for any $b$ in $B$, $\tilde{\sigma}(gB) = 1$ for all cosets $gB$. Therefore, for all $g$ in $SL_2$
\begin{displaymath}
...
\end{lemma}
\begin{proof}
As in the previous example, let $x$ be an element of the kernel of the map $H^1(i):H^1(B,V)\rightarrow H^1(U,V)$ and let $\sigma$ in $Z^1(B,V)$ be a representative for $x$ such that $\sigma|_U = 1$. Let $T$ be a maximal torus for $B$. For any $u$ in $U$ and $t$ in $T$ there is a $u'$ in $U$ such that
\begin{displaymath}
	ut = tu'.
\end{displaymath}
