...
Using Definition TODO, we can define $Z^1(K, V)_{l \cdot \sigma}$ to be the collection of 1-cocycles from $K \rightarrow V$ where $K$ acts on $V$ via $l \cdot \sigma$. The formula for the action is then given by
\begin{align*}
	x.v = l \sigma(x) l^{-1} v l \sigma(x^{-1}) l^{-1},
\end{align*}
for all $x \in K, v \in V$. Evidently $Z^1(K, V)_{l \cdot \sigma} = z\left(\mathrm{Hom}(K, P)_{l \cdot \sigma}\right)$. The reason for the new notation for the action will become apparent presently, as we show that we have a map of 1-cohomomologies $H^1(K, V)_\sigma \rightarrow H^1(K, V)_{l \cdot \sigma}$.

	We show that we can apply Lemma TODO. To this end, let $K' = K$, $V' = V$ and let $\zeta:K' \rightarrow K$ be defined by
	\begin{align*}
		\zeta = \mathrm{id}:K \rightarrow K,
	\end{align*}
	and $\xi = \xi_l: V \rightarrow V'$ be defined by
	\begin{align*}
		\xi_l(v) = lvl^{-1},
	\end{align*}
	for all $v \in V$. Now we show that $\xi_l$ satisfies Equation TODO. Let $x \in K, v \in V$. Then
	\begin{align*}
	...
	\end{align*}
	Therefore the map $Z^1(\zeta, \xi_l):Z^1(K, V)_\sigma \rightarrow Z^1(K, V)_{l \cdot \sigma}$ defined by 
	\begin{align*}
...
\begin{proof}
	...
	\begin{align*} \varsigma(x) = v\rho(x)v^{-1}, \end{align*}
		...
		\end{align*}
		shows that Equation \ref{cl.h1} defines an action of $C_L(\sigma(K))$ on $H^1(K, V)_\sigma$.
\end{proof}
	\begin{definition}
	...
	We have the following Lemma, which shows that the map
	\begin{align*}
		h:\mathrm{Hom}(V, P)_\sigma/V \rightarrow H^1(K, V)_\sigma,
	\end{align*}
	of Lemma TODO descends to give a bijection from
	\begin{align*}
...
\begin{lemma} Let $R \subset \mathrm{Hom}(K, P)$. Suppose $R = P \cdot \rho$ for some $\rho \in R$. Then $R^L = L\cdot \rho^L$.
	More generally, if $R = P \cdot R$ then $R^L = L \cdot R^L$.
\end{lemma}
\begin{proof}
\end{proof}

\begin{lemma} Let $R \subset \mathrm{Hom}(K, P)$ and suppose that $R = P \cdot R$. Then for all $\sigma \in \mathrm{Hom}(K, L)$
	\begin{align*}
...
By (i) there exists a finite set $\mathcal{S} \subset \mathrm{Hom}(\Gamma, L)$ such that
\begin{align} \label{finiterl}
R^L = \bigcup_{\tau \in \mathcal{S}} L \cdot \tau.
\end{align}

Define $L \cdot R_\sigma = \{L \cdot \rho \,|\, \rho \in R_\sigma\}$. Evidently $L \cdot R_\sigma$ is contained in a finite union of $P$-conjugacy classes.


Now let $\rho \in R$. By Equation \ref{finiterl} there exists $l \in L$, $\tau \in \mathcal{S}$ such that $\rho^L = l \cdot \tau$. Hence $l^{-1} \cdot \rho^L = \tau$, so $\rho \in L \cdot R_\tau$. This shows that $R$ is contained in a finite union of $P$-conjugacy classes. The reverse inclusion is satisfied since $R = P \cdot R$.

This completes the proof.
\end{proof}

\begin{definition} Let $K' < K$ and let $\zeta$ be the inclusion map $K' \hookrightarrow K$. We denote by $\rho^\zeta$ the homomorphism $K' \rightarrow P$ defined by
	\begin{align*}
