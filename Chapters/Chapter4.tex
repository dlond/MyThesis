...
\begin{lemma} Let $K, K', V, V', \zeta, \xi$ satisfy the requirements of Lemma \ref{h1maps}. Then the map $H^1(\zeta, \xi):H^1(K, V)_\sigma \rightarrow H^1(K', V')_{\sigma\circ\zeta}$ descends to give a well-defined map
	\begin{align*}
		\widetilde{H^1}(\zeta, \xi): H^1(K, V)_\sigma/C_L(\sigma) \rightarrow H^1(K', V')_{\sigma\circ\zeta}/C_L(\sigma\circ \zeta),
	\end{align*}
defined by
	\begin{align*}
		\widetilde{H^1}(\zeta, \xi)(\widetilde{\psi}(\psi(\alpha))) = \widetilde{\psi'}\left(H^1(\zeta, \xi)(\psi(\alpha))\right).
	\end{align*}
\end{lemma}
\begin{proof}
\end{proof}
