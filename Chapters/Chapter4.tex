...
\begin{proof}
	...
	\begin{align*} \tau(x) = v\rho(x)v^{-1}, \end{align*}
		...
		\begin{align*}
			...
			&= c \rho(x) c^{-1},
		\end{align*}
		for all $x \in K$. Therefore $\phi(\xi_c \circ \tau) = \phi(\xi_c \circ \rho)$ and hence the map defined by Equation \ref{vcl.hom} is well-defined. It is clear that the group action axioms are satisfied, so Equation \ref{vcl.hom} defines an action of $C_L(\sigma(K))$ on $\mathrm{Hom}(K, P)_\sigma$.
