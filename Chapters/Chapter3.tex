...
\end{align*}
for all $x \in K$, is called a \emph{1-coboundary}. We denote by $B^1\left(K, V\right)$ the collection of all 1-coboundaries. 
\end{definition}
...
\end{align*}
Again, this makes little sense in the non-abelian setting. Instead we have the following.
...
The following Lemma is useful when showing that $H^1(\zeta, \xi)$ is injective (cf. Lemma \ref{brown}, Lemma \ref{lem:sl2_b_inj}).
...
Let $V$ be a vector space over $k$, $\mathrm{char}(k) = p$. Let $\Gamma$ be a finite group that acts linearly on $V$, and let $\Gamma_p$ be a \emph{Sylow $p$-subgroup} of $\Gamma$. Let $\zeta$ be the inclusion of $\Gamma_p$ in $\Gamma$, and $\xi$ the identity map on $V$. The map 
\begin{align*}
...
Let $w = \sum_{i=1}^l \beta(\gamma_i)$. Then
\begin{align*}
...
Since $\mathrm{gcd}(l, p) = \mathrm{gcd}\left([\Gamma_p:\Gamma], p\right) = 1$, $l \neq 0$. Therefore, by Lemma \ref{vspace} $\beta \in B^1(\Gamma, V)$ which proves $\mathrm{Ker}\left(H^1(\zeta, \xi)\right)$ is trivial.
\end{proof}
...
\begin{align*}
	\xymatrix{
		...
		H^1(SL^2(\mathbb{F}_{p^r}), V)\, \ar[r]_{H^1(\zeta_r, \xi)} &\, H^1(U_2(\mathbb{F}_{p^r}), V).
	}
\end{align*}
...
for any $v \in V$. For each $x\in SL_2(\mathbb{F}_{p^r})$ define the morphism $f_x:V\rightarrow V$ by
	\begin{align*}
		f_x(v) = v^{-1} \beta(x) \left(x \cdot v\right).
	\end{align*}
...
Consider sequence $\left\{C_{r}\right\}_{r \in \mathbb{N}} \subset V$ defined by
	\begin{align*}
		\left\{C_{r}\right\} = \{v \in V \,|\,\forall x\in SL_2(\mathbb{F}_{p^{r}}),\, f_x(v) = e\},
	\end{align*}
...
\begin{align*}
	&\{v \in V \,|\, \forall x \in SL_2(k), f_x(v) = e\} \\
