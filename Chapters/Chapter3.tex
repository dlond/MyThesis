...
\begin{corollary} \label{zlinear} Let $K,K',V,V',\zeta,\xi$ satisfy the requirements of Lemma \ref{h1maps} and suppose $V,V'$ are abelian (respectively, vector spaces). Then $Z^1(\zeta, \xi)$ is a homomorphism (respectively, a linear map).
\end{corollary}
\begin{proof}
...
\begin{example} Following Example \ref{ab_example}, suppose now that $V$ is an algebraic group, not necessarily a vector space. Let $B$ be a Borel subgroup of $SL_2(k)$ and let $\zeta: B \rightarrow SL_2(k)$ be the inclusion map. Then $H^1(\zeta): H^1(SL_2(k), V)\rightarrow H^1(B, V)$ is injective.
  \label{sl2_b_inj}
\end{example}
\begin{proof}
