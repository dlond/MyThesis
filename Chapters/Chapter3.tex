...
We will show that given a representation $\rho:\Gamma\rightarrow G$ there exists a representation $\sigma$ that is $G$-conjugate to $\rho$ and that fits one of only finitely many diagrams of the form:
\begin{displaymath}
	\xymatrix{
...
Since $Q$ is a minimal parabolic containing $\rho'(\Gamma)$ $\rho_0$ is $M$-irreducible [reference] and therefore $M$-completely reducible. Hence there exists an $m\in M$ such that $\sigma_0 = m\cdot \rho_0$ where $\sigma_0\in\{\sigma^{(i)}_{0,j}\}$ [Theorem]. Let $\sigma = m\cdot \rho' = mg\cdot \rho$. This verifies what we set out to show.
...
\begin{lemma} Let $\{\rho_\mu\, |\, \mu \in I\}$ be a collection of representations $\Gamma\rightarrow P$ for a fixed parabolic subgroup $P<G$. The following statements are equivalent:
	\begin{itemize}
