...
\begin{align*}
	...
	&= \left(\begin{matrix}a & 0\\0 & a^{-1}\end{matrix}\right) \cdot
	\sigma\left(\begin{matrix}1 & a^{-1}b\\0 & 1\end{matrix}\right) \\
	&\subset \sigma\left(U_2(k)\right)\quad(\textrm{by Equation \ref{eqn:t_act}}).
\end{align*}
...
\end{itemize}
Then for all $a\in k^*, b\in k$, there exist $\mu_\delta\in k$ such that
\begin{align*}
...
\begin{conjecture}\label{bigclaim} Let $G$ be a reductive group, $P_\alpha$ a minimal parabolic subgroup of $G$ with unipotent radical $V_\alpha$. Let $x \in H^1(SL_2(k), V_\alpha)_{\rho_r}$. Then there exists $\sigma \in Z^1(SL_2(k), V_\alpha)_{\rho_r}$ such that $\psi(\sigma) = x$. Furthermore, for all $a\in k^*, b\in k$
\begin{align*}
	\sigma\left(\begin{matrix}a & b \\0 & a^{-1}\end{matrix}\right) = \prod_{\delta\in\mathcal{D}} \epsilon_\delta\left(a^{\langle\delta,\alpha\rangle p^r}x_\delta(a^{-1}b)\right),
\end{align*}
