...
\section{This won't stay up here}
\begin{lemma} $H^1(SL_2, V)\rightarrow H^1(U, V)$ is injective.
\end{lemma}
\begin{proof}
By [ref Humphreys] $H^1(SL_2, V)\rightarrow H^1(B, V)$ is injective, so we just need to show that $H^1(B, V)\rightarrow H^1(U, V)$ is injective. 

Let $x$ be an element of the kernel of the map $H^1(i):H^1(B,V)\rightarrow H^1(U,V)$ and let $\sigma$ be a representative in $Z^1(B,V)$ for $x$. Since $Z^1(i)(\sigma)$ represents the class containing the trivial 1-cocycle we may assume that $\sigma(u) = 0$ for all $u$ in $U$. There exists a $v$ in $V$ such that $\sigma(t) = \chi(t)$ for all $t$ in $T$. Define
\begin{eqnarray*}
	\sigma'(g) = v^{-1}\sigma(g) g\cdot v.
\end{eqnarray*}
We have adjusted $\sigma$ by a 1-coboundary to yield a 1-cocycle $\sigma'$ of the same 1-cohomology class that is zero on $T$. 

Since $\sigma(u) = 0$ for any $u\in U$, and for any $t\in T$ there is a $u'$ such that $ut = tu'$
\begin{eqnarray*}
	\sigma(ut) &=& \sigma(tu')\\
	\Rightarrow \sigma(u)u\cdot \sigma(t) &=& \sigma(t) t \cdot \sigma(u')\\
	\Rightarrow u\cdot\sigma(t) &=& \sigma(t).
\end{eqnarray*}
Hence
\begin{eqnarray*}
\sigma'(u) &=& v^{-1}\sigma(u)u\cdot v\quad[\textrm{phew I made a mistake here}]\\
	&=& v^{-1}\sigma(u)\cdot v\\
	&=& v^{-1} v\\
	&=& 0.
\end{eqnarray*}
