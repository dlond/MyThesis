...
We are interested in finding the general form of a 1-cocycle $\varsigma \in Z^1(SL_2(k), V_\alpha)_{\rho_r}$. Then we can apply the canonical projection $\psi:Z^1(SL_2(k), V_\alpha)_{\rho_r} \rightarrow H^1(SL_2(k), V_\alpha)_{\rho_r}$ to calculate the 1-cohomology (cf. Chapter \ref{Chapter6}).
...
Unlike in Remark \ref{g2rem} it is not obvious whether an irreducible root sytem contains $C_3$ or not.
\end{remark}
\begin{proof}
...
For case (iv) $\delta_1, \delta_2$ again lie in a $B_2$ subsystem, and $\delta_i, \gamma$ lie in an $A_2$ subsystem $(i = 1, 2)$. Then the roots $\delta_i, \gamma$ $(i=1, 2)$ are the same length, so $\gamma$ is a short root. Therefore, if $\delta_1, \delta_2, \gamma$  lie in a rank 3 subsystem $X$, $X$ contains two short root $A_2$ subsystems and a $B_2$ subsystem. TODO \ldots Therefore $X=C_3$ so we can rule out case (iv).
\end{proof}
...
We return to the 1-cohomology calculation but assume that $\Phi$ does not contain $G_2$ or $C_3$.
...
Let $\delta_1, \delta_2 \in \mathcal{D}$. Since $\langle \delta_i, \alpha \rangle > 0$ $(i=1,2)$, $\delta_1 + \delta_2$ is not a root (Proposition \ref{uabelian}), so $U_{\delta_1}$ and $U_{\delta_2}$ commute. Therefore $\sigma\left(\begin{matrix}1 & * \\ 0 & 1\end{matrix}\right)$ lies in a product of commuting root groups. Furthermore, for any $a, b\in k$ with $a\neq 0$
\begin{align*}
\sigma\left(\begin{matrix} a & ab \\ 0 & a^{-1}\end{matrix}\right)
&= \left(\sigma\left(\begin{matrix} a & 0 \\ 0 & a^{-1}\end{matrix} \right)\right)\left(\begin{matrix} a & 0 \\ 0 & a^{-1}\end{matrix} \right) \cdot
\sigma\left(\begin{matrix} 1 & b \\ 0 & 1\end{matrix}\right) \\
&= \left(\begin{matrix} a & 0 \\ 0 & a^{-1}\end{matrix} \right) \cdot
\sigma\left(\begin{matrix} 1 & b \\ 0 & 1\end{matrix}\right) \\
&= \prod_{\delta\in\mathcal{D}} \epsilon_\delta\left(a^{\langle \delta, \alpha \rangle p^r}x_\delta\left(b\right)\right)\quad(\textrm{Equation }\ref{eqn:t_act}).
\end{align*}
Therefore $\sigma\left(\begin{matrix}* & * \\ 0 & *\end{matrix}\right)$ lies in a product of commuting root groups. This proves (ii).
\end{proof}

\begin{lemma}[Second Main Lemma]\label{lem:second}
Suppose $\Phi$ does not contain $G_2$ or $C_3$. If $p = 2$ then
\begin{align*}
	\sigma\left(\begin{matrix}* & *\\0 & *\end{matrix}\right) \subset \langle U_\delta \,|\, \delta \in \Phi^+ - \{\alpha\}, \langle \delta, \alpha \rangle = 1\textrm{ or }2\rangle,
\end{align*}
otherwise
\begin{align*}
	\sigma\left(\begin{matrix}* & *\\0 & *\end{matrix}\right) \subset \langle U_\delta \,|\, \delta \in \Phi^+ - \{\alpha\}, \langle \delta, \alpha \rangle = 2 \rangle.
\end{align*}
Furthermore, there exists $\mu \in k^*$ such that, if $p = 2$ then
\begin{align*}
	x_\delta(\lambda) = \mu\lambda^{2^{r - 2 + \langle \delta, \alpha \rangle}},\quad\forall\lambda\in k,
\end{align*}
otherwise
\begin{align*}
	x_\delta(\lambda) = \mu\lambda^{p^r},\quad\forall\lambda\in k.
\end{align*}
\end{lemma}
\begin{proof}
...
In the next Chapter we investigate the counterexamples to Propositions \ref{ufixes} and \ref{uabelian} as pointed out in the respective Remarks \ref{g2counter} and \ref{c3counter}. In particular we show that for $G = G_2$, $H^1(SL_2(k), V_\alpha)_{\rho_r}$ is trivial and $\sigma\in Z^1(SL_2(k), V_\beta)_{\rho_r}$ satisfies the two main Lemmas. We also show that for $G=C_3$, $\sigma\in Z^1(SL_2(k), V_\alpha)_{\rho_r}$ 

In view of these examples, we modify the two main Lemmas of this Chapter appropriately.
