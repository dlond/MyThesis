...
\begin{lemma}
	Let $G$ be a reductive group over a closed field of positive characteristic $p$ and let $\Gamma = SL_2(k)$. Then for each $\alpha \in \Delta$ the image $\sigma \in Z^1(SL_2(k), V_\alpha)$ lies in a product of commuting root groups.
\end{lemma}
\begin{proof}
  The only cases not covered by Corollary \ref{cor:im_ab} are shown by way of example (Examples \ref{eg:c3}, \ref{eg:g2}).
\end{proof}
